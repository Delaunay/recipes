    ...
    def crawl_dynamic_endpoints(self):
        ...
        for ingredient in self.ingredients_data:
            ...
            if ingredient_id:
                ...
                # Crawl new unit conversion endpoints
        logger.info("Crawling unit definition endpoints...")
        all_units = self.available_units if self.available_units else []

        # Crawl unit definitions for all available units
        for unit in all_units:
            endpoint = f"/unit/definition/{unit}"
            data = self.fetch_json(endpoint)
            if data:
                self.save_json_file(endpoint, data)

        # Crawl ingredient endpoints for unit conversion
        logger.info("Crawling ingredient endpoints for unit conversion...")
        for ingredient in self.ingredients_data:
            ingredient_id = ingredient.get('id')
            if ingredient_id:
                endpoint = f"/ingredients/{ingredient_id}"
        ...
        # Crawl unit conversion factor endpoints
        logger.info("Crawling unit conversion factor endpoints...")
        for from_unit in all_units:
            for to_unit in all_units:
                if from_unit != to_unit:
                    endpoint = f"/unit/convert/{from_unit}/{to_unit}"
                    data = self.fetch_json(endpoint)
                    if data:
                        self.save_json_file(endpoint, data)
    ...
    def crawl_api_endpoints(self):
        ...
        for endpoint in self.api_endpoints:
            ...
            if data:
                ...
                elif endpoint == "/units/available":
                    self.available_units = data
                    logger.info(f"Stored {len(data) if isinstance(data, list) else 'non-list'} available units")
                elif endpoint == "/units/available/volume":
                    self.volume_units = data
                    logger.info(f"Stored {len(data) if isinstance(data, list) else 'non-list'} volume units")
